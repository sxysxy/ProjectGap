...
void RGSSPlayer::LoadRGSS() {
    ...
    if (!hRGSSCore) {
        MessageBoxError(hWnd, szTitle, L"º”‘ÿRGSS∫À–ƒø‚ ß∞‹£¨¬∑æ∂£∫ %s£¨≥Ã–Ú÷’÷π£°", szLibrary);
        ExitProcess(0);
    }
    
#define __get_ptr(fn) lpfn##fn = (fn)GetProcAddress(hRGSSCore, #fn);  \
       if (!lpfn##fn) {  \
           MessageBoxError(hWnd, szTitle, L"º”‘ÿRGSS∫À–ƒø‚∫Ø ˝ %s  ß∞‹£¨≥Ã–Ú÷’÷π£°", L#fn);  \
           ExitProcess(0); \
       }
...
#undef __get_ptr
    ...
    if (!lpfnRGSSSetupRTP(szIniPath, szRtpName, 1024)) {
        MessageBoxError(hWnd, szTitle, L"’“≤ªµΩ RGSS-RTP : %s", szRtpName);
        ExitProcess(0);
    }
...
void RGSSPlayer::InitD3DContext() {
    renderer = SDL_CreateRenderer(window, 0, SDL_RENDERER_ACCELERATED | SDL_RENDERER_TARGETTEXTURE);
                                            //”≤º˛º”ÀŸ£¨ø…“‘‰÷»æÃ˘Õº
    if (!renderer) {
        MessageBoxError(hWnd, szTitle, L"≥ı ºªØD3D¢·‰÷»æ∆˜ ß∞‹£¨≥Ã–Ú÷’÷π£°");
        ExitProcess(0);
    }
